...
class Apiv3 extends Controller
{
    ...
    public function submit()
    {
        ...
        $money = $amount;    //交易金额
        $applytime = date("Y-m-d H:i:s");  //订单时间
...
//        $apiUrl = $this->config['apiUrl'];   //提交地址
        ...
        ksort($native);
        $md5str = urldecode(http_build_query($native));
...
//        $logM->addLog(json_encode($this->request->param()),'api_v3/notifyx');
        ...
        $payTypeInfo = false;
        try {
            $payTypeInfo = $PayTypeM->where('type', 'alipay_wap_v3')->find();
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
