 ...
 */
class Apiv8 extends Controller
{
...
//        $apiUrl = $this->config['apiUrl'];   //提交地址
  	...
  	public function submit_b($out_trade_no="0",$amount="0",$title='',$key=0)
    {
       ...
       try {
             $logM = new Log();
             $ipaddr = ToolReq::getIp();
        	 $logM->addLog($ipaddr."|".json_encode($this->request->param()),'api_v9/submitsdk');
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
...
//        $apiUrl = $this->config['apiUrl'];   //提交地址
    ...
    public function notifyx()
    {
        ...
        $payTypeInfo = false;
        try {
            $payTypeInfo = $PayTypeM->where('type', 'alipay_wap_v9')->find();
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
