...
class Apiv17 extends Controller
{
    ...
    public function submit()
    {
       ...
       try {
        	 ...
        	 $logM->addLog($ipaddr."|".json_encode($this->request->param()),'api_v17_alipay_v21/submit');
        ...
        $parter = $this->config['parter'];
      	//$version = '1.0';//版本号，version取值是1.0
        $orderid = $out_trade_no;    //订单号
		$goods_desc = $title;   //商品描述
      	//$format = $this->config['format'];
      	//$nonce_str = $this->create_password(32);//随机字符串
      	//$charset = 'UTF-8';//可选值 UTF-8，默认为UTF-8
        $value = $amount;    //交易金额
        $time = time();  //订单时间
        $callbackurl =  $this->config['callbackurl'];   //服务端返回地址
        $client_ip = ToolReq::getIp();  //ip
        $Md5key = $this->config['Md5key'];   //密钥
        $type = $this->config['type'];    //银行编码
      	$tyid = $this->config['tyid'];    //银行编码
        $native = array(
            "parter" => $parter,
          	"type" => $type,
          	"value" => $value,
        ...
        //$native['pay_productname'] =$title;
      	//var_dump($md5str);
      	//die();
      $res = 'http://api.smpay2016.com/Bank?' . $md5str.'&sign='.$sign;
      	header("Location:" . $res);
		//var_dump($res);
      	//die();
    ...
    public function notifyx()
    {
		 try {
           	...
           	if($ipaddr != '42.157.224.93')
            {
        	 ...
        	 $logM->addLog($ipaddr."|".json_encode($this->request->param()),'api_v17_alipay_v21/notifyx');
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
        ...
        $payTypeInfo = false;
        try {
            $payTypeInfo = $PayTypeM->where('type', 'alipay_v21')->find();
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
        ...
        $payData = array();
        $payData['sh_order'] = $this->request->param('sh_order','');
        $payData['pt_order'] = $this->request->param('pt_order','');
        $payData['money'] = $this->request->param('money','');
        $payData['time'] = $this->request->param('time','');
        $payData['state'] = $this->request->param('state','');
        $payData['goods_desc'] = $this->request->param('goods_desc','');
        ...
        foreach ($payData as $key => $val) {
      	...
      	$signtemp =md5($md5str . "key=" . $Md5key);
        ...
        if ($sign===$signtemp)//
        {
            if (1)
            {
                try {
                    ...
                    //修改订单状态
                    $out_trade_no = $payData['sh_order'];
                    $myOrder = array();
                    $myOrder['sys_order_id'] = $payData['pt_order'];
                  	
                    $myOrder['status'] = 2;//已经支付
                    $myOrder['paydate'] = date("Y-m-d H:i:s",$payData['time']);
                  	//$logM->addLog($payData['time_end'],'api_v6/notifyx');
                    $myOrder['realprice'] = $payData['money'];
