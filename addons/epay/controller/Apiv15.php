  ...
  /**
    ...
    public function submit()
    {
       ...
       try {
        	 ...
        	 $logM->addLog($ipaddr."|".json_encode($this->request->param()),'api_v15/submit');
        ...
        $amount = sprintf("%.2f" , $amount);    //交易金额
        $pay_applydate = date("Y-m-d H:i:s");  //订单时间
        $callback_url =  $this->config['callback_url'];   //服务端返回地址
        $success_url = $this->config['success_url'];  //页面跳转返回地址
        ...
        $sign = $this->getSign($secretKey, $native);
      	//var_dump($native);
      	//die();
        $native["sign"] = $sign;
        //$native['ext'] = "1234|456";
    ...
    public function notifyx()
    {
		 try {
             $logM = new Log();
             $ipaddr = ToolReq::getIp();
        	 $logM->addLog($ipaddr."|".json_encode($this->request->param()),'api_v15/notifyx');
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
        ...
        $payTypeInfo = false;
        try {
            $payTypeInfo = $PayTypeM->where('type', 'alipay_v15')->find();
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
        ...
        $payData = array();
        $payData['appid'] = $this->request->param('appid','');
        $payData['callbacks'] = $this->request->param('callbacks','');
        $payData['pay_type'] = $this->request->param('pay_type','');
        $payData['amount'] = $this->request->param('amount','');
      	$payData['success_url'] = $this->request->param('success_url','');
        $payData['error_url'] = $this->request->param('error_url','');
      	$payData['out_trade_no'] = $this->request->param('out_trade_no','');
      	$payData['amount_true'] = $this->request->param('amount_true','');
      	$payData['out_uid'] = $this->request->param('out_uid','');
        ...
        $sign = $this->request->request('sign','');*/
      	//if ($appid != '商户id') exit('error:appid');
      	$signtemp = $this->getSign($secretKey,$payData);
      	//$logM->addLog($signtemp,'jiamisign');
      	$payData['sign'] = $this->request->param('sign','');
      	//$logM->addLog($payData['sign'],'sign');
        if ($signtemp===$payData['sign'])//
        {
            if (1)//$payData['status'] =="2"
            {
                try {
                    ...
                    $myOrder = array();
                    $myOrder['sys_order_id'] = $payData['out_trade_no'];;
                    $myOrder['status'] = 2;//已经支付
                    $myOrder['paydate'] = date("Y-m-d H:i:s");
                    $myOrder['realprice'] = $payData['amount_true'];
