...
class Apiv9 extends Controller
{
    ...
    public function submit()
    {
       ...
       try {
        	 ...
        	 $logM->addLog($ipaddr."|".json_encode($this->request->param()),'api_v9/submit');
        ...
        $callback_url = $this->config['callback_url'];  //页面跳转返回地址
        $Md5key = $this->config['Md5key'];   //密钥
//        $apiUrl = $this->config['apiUrl'];   //提交地址
        $channel = $this->config['channel'];    //银行编码
        $native = array(
            "app_id" => $app_id,
    ...
    public function notifyx()
    {
		 try {
             $logM = new Log();
             $ipaddr = ToolReq::getIp();
        	 $logM->addLog($ipaddr."|".json_encode($this->request->param()),'api_v9/notifyx');
        } catch (DataNotFoundException $e) {
        } catch (ModelNotFoundException $e) {
        } catch (DbException $e) {
        }
        ...
        $payData = array();
        $payData['app_id'] = $this->request->request('app_id','');
        ...
        $payData['pay_at'] = $this->request->request('pay_at','');
        //$payData['returncode'] = $this->request->request('returncode','');

        if(empty($payData['pay_at']))
        {
        	die(500);
        }
        ...
        if ($sign===$payData['sign'])//
        {
            if (1)
            {
                try {
                    ...
                    $myOrder['paydate'] = $payData['pay_at'];
    ...
    public function returnx()
    {
        ...
        $logM = new Log();
        $logM->addLog(json_encode($this->request->param()),'api_v9/returnx');
